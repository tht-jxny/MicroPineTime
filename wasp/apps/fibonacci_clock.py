...
MONTH = "JanFebMarAprMaiJunJulAugSepOktNovDec"
...
class FibonacciClockApp:
    ...
    def update(self):
        ...
        self.calculateColors(
            now[3], now[4] // 5
        )  # Clock can only display every 5 minutes

        draw.fill(
            x=71, y=41, w=23, h=23, bg=self.color_codes[self.field_colors[4]]
        )  # 1 field
        draw.fill(
            x=71, y=66, w=23, h=23, bg=self.color_codes[self.field_colors[3]]
        )  # 1 field
        draw.fill(
            x=21, y=41, w=48, h=48, bg=self.color_codes[self.field_colors[2]]
        )  # 2 field
        draw.fill(
            x=21, y=91, w=73, h=73, bg=self.color_codes[self.field_colors[1]]
        )  # 3 field
        draw.fill(
            x=96, y=41, w=123, h=123, bg=self.color_codes[self.field_colors[0]]
        )  # 5 field
        ...
        draw.string("{}. {} {}".format(now[2], month, now[0]), 0, 185, width=240)
